...
- Pan, zoom, opacity control, and background image locking.
- Scale calibration from a drawn reference line with real feet and inches.
- Click-to-draw straight and angled fence runs with draggable endpoints, run labels, length labels, total footage, manual length overrides, and per-run post spacing.
- Automatic post placement with editable post type, material/size, notes, and manual post add/move/delete.
...
- Custom gates including walk, double, sliding, cantilever, and custom gates with fabrication and customer-facing notes. Cantilever gates include opening, tail, overall length, roller/post, receiver, catch, and bracing fields.
